    ...
    return (
      <div className="App">
        <header className="App-header">
          {loading && <ProgressBar
            className={progressFadeOutAnimation}
          ...
          {introducing && !started && <button
            className={`nes-btn is-primary ${zoomIn}`}
            variant="primary">{`Esun Lucky Wheel for: ${this.state.name}`}</button>}
          {introducing && !started && <button
            type="button"
            className={`nes-btn is-warning ${zoomIn}`}
            onClick={() => this.setState({ startPressed: true })}>Warning</button>}
          {started && <Circle
